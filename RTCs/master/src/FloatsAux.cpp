...
/* Split a string of comma-separated floats and store them in a vector of floats */
...
/* Generate a comma-separated string of floats from a vector of them */
std::string floats2string(std::vector<float> f)
{
 std::string s;
 std::ostringstream stream;
 std::vector<float>::iterator it = f.begin();
	while(1){
		stream << *it;
		if(++it != f.end()) {
			stream << ",";
